...
prep_graph <- function(file=here::here("data","contributors.xlsx")){ 
    ...
    nodes_dt_agg <- nodes_dt[,list(entity=toString(unique(entity)),
                                   ...
                                   cluster_str=paste(unique(cluster),
                                                     collapse =";"),
                                   subcluster=unique(subcluster),
    ...
    extra_edges1 <- lapply(unique(nodes_dt_agg$cluster),
                          function(clust){
        nodes <- unique(nodes_dt_agg[cluster==clust,]$key)
        cj <- data.table::CJ(from=nodes,to=nodes,unique = TRUE)     
        cj[from!=to][,hidden:=TRUE]
    }) |> data.table::rbindlist()
    extra_edges2 <- lapply(unique(nodes_dt_agg$subcluster),
                          function(clust){
          nodes <- unique(nodes_dt_agg[subcluster==clust,]$key)
          cj <- data.table::CJ(from=nodes,to=nodes,unique = TRUE)     
          cj[from!=to][,hidden:=TRUE] 
    }) |> data.table::rbindlist()
    edges_dt <- data.table::rbindlist(list(edges_dt,
                                           extra_edges1,
                                           extra_edges2), 
                                      fill = TRUE)
