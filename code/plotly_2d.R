plotly_2d <- function(g,
                    ...
                    showlegend = TRUE,
                    show_plot = TRUE,
                    save_path = tempfile(fileext = "network_2d.html"),
                    verbose = TRUE){
    
    devoptera::args2vars(plotly_2d,
                         path="code/",
                         pattern="plotly_2d",
                         reassign = TRUE)
    d <- HPOExplorer:::igraph_to_plotly(g,dim = 2)
    vdf <- d$vertices
    vdf[,label:=gsub("\n","<br>",name)]
    edf <- d$edges
    ...
    #### Add layout ####
    scene <- lapply(stats::setNames(c("xaxis","yaxis"),
                                    c("xaxis","yaxis")),
